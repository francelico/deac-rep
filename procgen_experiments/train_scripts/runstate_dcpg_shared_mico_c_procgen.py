...
class Args:
  ...
  vdist_coef_pol_phase: float = 1.0
  "coefficient for the value function loss during the policy phase"

  # Markov loss arg
  markov_coef_a: float = 0.0
  "coefficient for the Markov forward-inverse dynamics loss during the auxiliary phase for the Actor"

  # MICo arguments #NOTE: target network is forced to hard updates at the end of aux phase in this version
  ...
  mico_coef_pol_phase: float = 0.0
  "the coefficient for the MICo loss during the policy phase"
  mico_coef_aux_phase: float = 0.0
  "the coefficient for the MICo loss during the auxiliary phase"
...
def make_ppg_model(args, envs, key, print_model=False):
  key, actor_base_key, policy_head_key, value_head_key, dyna_head_actor_key = jax.random.split(key, 5)
...
class AgentTrainStateWithTarget:
  ...
  def apply_gradients(self, grads):
    ...
    return AgentTrainStateWithTarget(actor_state, actor_target_params=self.actor_target_params)
...
if __name__ == "__main__":
  ...
  def update_target_params(target_p, online_p, tau):
  ...
  def policy_phase_loss_fn(params, target_params, obs, next_obs, rewards, actions, behavior_logprobs,
    ...
    # Value loss
    old_value_preds, target_reps_a = jax.lax.stop_gradient(get_value_and_rep(target_params.actor_params, obs))
    value_preds = ValueHead().apply(params.actor_params.value_head_params, reps_a).squeeze(-1)
    v_loss = value_loss_fn(value_preds, old_value_preds)

    if args.enable_mico_pol_phase:
      # MICo loss actor
      target_nreps_a = get_representation(target_params.actor_params, next_obs)
      mico_loss, mico_stats = mico_reward_diff_loss_fn(rewards, reps_a, target_reps_a, target_nreps_a,
                                                       remove_same_rollout_pairs=args.remove_same_rollout_pairs)
    else:
      mico_loss, mico_stats = jnp.zeros_like(v_loss), {}

    loss = pg_loss - args.ent_coef * entropy_loss + args.vdist_coef_pol_phase * v_loss + args.mico_coef_pol_phase * mico_loss
    stats = {
  ...
  def auxiliary_phase_loss_fn(params, target_params, obs, next_obs, dones, rewards, target_values, target_logits, actions, key):

    new_logits, value_preds, reps_a = get_logits_valuepred_rep(params.actor_params, obs)
    ...
    aux_phase_loss = vloss + args.markov_coef_a * dyna_loss_a + args.bc_coef * kl_policy_bc + args.mico_coef_aux_phase * mico_loss

    stats = {
      'losses/auxiliary_phase_loss': aux_phase_loss,
      'losses/kl_policy_bc': kl_policy_bc,
      'losses/auxiliary_phase_value_loss': vloss,
      'losses/auxiliary_phase_mico_loss': mico_loss,
    ...
    if args.enable_aux_vf_loss:
      stats.update(repmetric_util.compute_nn_latent_out_stats(reps_a, value_preds, metric_fn_out=utils.absolute_diff,
  ...
  def run_auxiliary_phase_single_device(
    ...
    def aux_phase_epoch(carry, _):
      ...
      def aux_phase_minibatch(carry, minibatch):
        agent_state, key = carry
        mb_obs, mb_last_obs, mb_dones, mb_rewards, mb_target_values, mb_target_logits, mb_actions = minibatch
        ...
        grads = jax.lax.pmean(grads, axis_name="local_devices")
        agent_state = agent_state.apply_gradients(grads=grads)
        agent_state = agent_state.replace(actor_target_params=agent_state.get_params().actor_params)
