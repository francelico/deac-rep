...
if __name__ == "__main__":
  ...
  update_actor_target_params = partial(update_target_params, tau=args.target_smoothing_coef)
  ...
  def policy_phase_loss_fn(params, target_params, obs, next_obs, rewards, actions, behavior_logprobs,
    ...
    # Value loss
    value_preds = ValueHead().apply(params.actor_params.value_head_params, jax.lax.stop_gradient(reps_a)).squeeze(-1)
    ...
    loss = pg_loss - args.ent_coef * entropy_loss + args.vf_coef_pol_phase * v_loss + args.mico_coef_pol_phase * mico_loss
    stats = {
  ...
  def auxiliary_phase_loss_fn(params, target_params, obs, next_obs, dones, rewards, target_values, target_logits, actions, key):
    ...
    new_actor_target_p = jax.tree_map(update_actor_target_params, target_params.actor_params, params.actor_params)
    ...
    return aux_phase_loss, (stats, new_actor_target_p, key)
  ...
  def run_auxiliary_phase_single_device(
    ...
    def aux_phase_epoch(carry, _):
      ...
      def aux_phase_minibatch(carry, minibatch):
        ...
        (aux_phase_loss, (stats_aux_phase, actor_target_params, key)), grads = \
          aux_loss_grad_fn(
            agent_state.get_params(),
            agent_state.get_target_params(),
            mb_obs,
        ...
        grads = jax.lax.pmean(grads, axis_name="local_devices")
        agent_state = agent_state.apply_gradients(grads=grads)
        agent_state = agent_state.replace(actor_target_params=actor_target_params)
