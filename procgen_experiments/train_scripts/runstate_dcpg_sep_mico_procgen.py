...
# TASKS
# [] Try zero init of value heads.
# [] MICo: Try MICo + orthogonal init of heads --kernel_init_method=ppg_orthogonal
# [] MICo: Try moving MICo loss 2 layers before output (as in metric DQN agent, right now it's one.)
# [] MICo if enabling MICo pol phase: try detaching value head gradient of critic head during policy phase
...
class Args:
  ...
  "Toggle the value prediction loss during the auxiliary phase"
  ...
  "coefficient for the value/mico feature distillation loss during policy phase"
  ...
  "use the same value targets for the aux value head and the critic during the auxiliary phase"
  # MICo arguments
...
def make_agent(args, envs, key, print_model=False):
  ...
  agent_state = AgentTrainStateWithTarget(actor_state, critic_state,
                                          actor_target_params=actor_params, critic_target_params=critic_params)

  return agent_state, modules, key

def make_ppg_model(args, envs, key, print_model=False):
  key, actor_base_key, policy_head_key, auxiliary_head_key, dyna_head_actor_key, \
  critic_base_key, value_head_key, dyna_head_critic_key = jax.random.split(key, 8)
...
def rollout(
  ...
  def get_model_outputs(
    ...
    aux_value_pred = ValueHead().apply(params.actor_params.auxiliary_head_params, actor_base)
    return obs, action, logprob, value_pred.squeeze(), aux_value_pred.squeeze(), key
  ...
  for update in range(initial_policy_version + 1, args.num_updates + 2):
    ...
    for _ in range(0, args.num_steps):
      ...
      cached_next_obs, action, logprob, value_pred, aux_value_pred, key = \
      ...
      storage.append(
        PolPhaseStorage(
          ...
          aux_values=jax.lax.stop_gradient(aux_value_pred),
          env_ids=env_id,
          rewards=next_reward,
    ...
    if update % args.log_frequency == 0:
      ...
      if args.eval_every_n_log_cycles > 0 and (update // args.log_frequency) % args.eval_every_n_log_cycles == 0:
        ...
        for _ in range(0, args.num_steps):
          ...
          inference_time_start = time.time()
          ev_next_obs, action, _, _, _, key = get_model_outputs(params, ev_next_obs, key)
...
if __name__ == "__main__":
  ...
  args.aux_batch_rollouts = args.num_envs * args.num_policy_phases
  args.aux_batch_size = args.batch_size * args.num_policy_phases
  ...
  assert args.aux_batch_rollouts >= args.aux_minibatch_size, "aux_batch_rollouts must be >= aux_minibatch_size"
  ...
  def update_target_params(target_p, online_p, tau=1.0):
  ...
  def mico_reward_diff_loss_fn(rewards, reps, target_reps, target_nreps, remove_same_rollout_pairs=False):
    if remove_same_rollout_pairs:
      rewards = rewards.reshape(args.num_rollouts_in_aux_mb, args.num_steps)
      reps = reps.reshape(args.num_rollouts_in_aux_mb, args.num_steps, -1)
      target_reps = target_reps.reshape(args.num_rollouts_in_aux_mb, args.num_steps, -1)
      target_nreps = target_nreps.reshape(args.num_rollouts_in_aux_mb, args.num_steps, -1)
  ...
  def policy_phase_loss_fn(params, target_params, obs, next_obs, rewards, actions, behavior_logprobs,
    ...
    #Separate DCPG:
    old_aux_value_preds, target_reps_a = jax.lax.stop_gradient(get_auxvalue_and_rep(target_params.actor_params, obs))
    aux_value_preds = ValueHead().apply(params.actor_params.auxiliary_head_params, reps_a).squeeze(-1)
    if args.enable_aux_vf_loss:
      aux_vloss = value_loss_fn(aux_value_preds, old_aux_value_preds)
    else:
    ...
    if args.enable_mico_pol_phase:
      ...
      mico_loss, mico_stats = mico_reward_diff_loss_fn(rewards, reps_c, target_reps_c, target_nreps_c)
      critic_loss = (1 - args.mico_coef_critic_pol_phase) * v_loss + args.mico_coef_critic_pol_phase * mico_loss
      ...
      v_dist_loss = (1 - args.mico_coef_actor_pol_phase) * aux_vloss + args.mico_coef_actor_pol_phase * mico_loss_a
    else:
      critic_loss = v_loss
      v_dist_loss = aux_vloss
      mico_loss, mico_stats = jnp.zeros_like(v_loss), {}

    loss = pg_loss - args.ent_coef * entropy_loss + args.v_dist_coef * v_dist_loss + critic_loss
    ...
    stats = {
      ...
      'losses/aux_value_head_loss_policy_phase': aux_vloss,
    }
  ...
  def run_policy_phase_single_device(
    ...
    if args.singular_value_targets:
      ...
      last_aux_value, _ = get_auxvalue_and_rep(agent_state.get_params().actor_params, last_obs)
      _, aux_target_values = compute_gae(storage.aux_values, last_aux_value, last_done, last_term, storage)
    ...
    def shuffle_storage(storage, advantages, target_values, aux_target_values, next_obs, key):
      def flatten(x):
      ...
      next_obs_mask = jnp.broadcast_to(flatten_storage.dones.reshape(-1, 1, 1, 1), flatten_next_obs.shape)
      ...
      return shuffled_storage, shuffled_advantages, shuffled_target_values, shuffled_aux_target_values, shuffled_next_obs

    def policy_phase_epoch(carry, _):
      agent_state, key = carry
      key, subkey = jax.random.split(key)
      shuffled_storage, shuffled_advantages, shuffled_target_values, shuffled_aux_target_values, shuffled_next_obs = \
        shuffle_storage(storage, advantages, target_values, aux_target_values, next_obs, subkey)
    ...
    return agent_state, stats_pol_phase, \
           jnp.hsplit(target_values, num_sharded_storages), jnp.hsplit(aux_target_values, num_sharded_storages), key
  ...
  def auxiliary_phase_loss_fn(params, target_params, obs, next_obs, dones, rewards, target_values, aux_target_values,
                              target_logits, actions, key):
    ...
    #Note: separate DCPG uses aux_target_values that get computed separately.
    if args.enable_aux_vf_loss:
      aux_vloss = value_loss_fn(aux_value_preds, aux_target_values)
    else:
    ...
    # MICo loss critic
    mico_loss_fn = partial(mico_reward_diff_loss_fn, remove_same_rollout_pairs=args.remove_same_rollout_pairs)
    target_reps_c = get_representation(target_params.critic_params, obs)
    target_nreps_c = get_representation(target_params.critic_params, next_obs)
    mico_loss, mico_stats = mico_loss_fn(rewards, reps_c, target_reps_c, target_nreps_c)
    critic_loss = (1 - args.mico_coef_critic_aux_phase) * vloss + args.mico_coef_critic_aux_phase * mico_loss

    # MICo loss aux head
    target_reps_a = get_representation(target_params.actor_params, obs)
    target_nreps_a = get_representation(target_params.actor_params, next_obs)
    mico_loss_a, mico_stats_a = mico_loss_fn(rewards, reps_a, target_reps_a, target_nreps_a)
    mico_stats.update({f"{k}_a": v for k, v in mico_stats_a.items()})
    vf_dist_loss = (1 - args.mico_coef_actor_aux_phase) * aux_vloss + args.mico_coef_actor_aux_phase * mico_loss_a
    ...
    return aux_phase_loss, (stats, key)
  ...
  def run_auxiliary_phase_single_device(
    ...
    # NOTE: this has no impact when MICo isn't enabled. This can be a design choice.
    # tau=1.0 => hard update
    target_p = jax.tree_map(update_target_params, agent_state.get_target_params(), agent_state.get_params())
    agent_state = agent_state.replace(actor_target_params=target_p.actor_params, critic_target_params=target_p.critic_params)

    def shuffle_rollouts(storage, last_obs, logits, key):
      ...
      def convert_data(x: jnp.ndarray):
        ...
        x = jnp.reshape(x, (args.num_aux_minibatches * args.gradient_accumulation_steps, -1) + x.shape[2:])
        return x
    ...
    def aux_phase_epoch(carry, _):
      ...
      def aux_phase_minibatch(carry, minibatch):
        agent_state, key = carry
        mb_obs, mb_last_obs, mb_dones, mb_rewards, mb_target_values, mb_aux_target_values, \
        mb_target_logits, mb_actions = minibatch
        ...
        (aux_phase_loss, (stats_aux_phase, key)), grads = \
          aux_loss_grad_fn(
            agent_state.get_params(),
            agent_state.get_target_params(),
            mb_obs,
    ...
    # tau=1.0 => hard update
    target_p = jax.tree_map(update_target_params, agent_state.get_target_params(),
                                       agent_state.get_params())
    agent_state = agent_state.replace(actor_target_params=target_p.actor_params, critic_target_params=target_p.critic_params)
  ...
  if not runstate.metadata['training_completed']:
    ...
    sharded_storages_aux_phase = []
    sharded_last_obss_aux_phase = []
    while True:
      ...
      (agent_state, stats_pol_phase, target_values, aux_target_values, learner_keys) = \
