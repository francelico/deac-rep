...
class Args:
  ...
  "coefficient for the Markov forward-inverse dynamics loss during the policy phase for the Actor"
...
if __name__ == "__main__":
  ...
  def policy_phase_loss_fn(params, target_params, obs, next_obs, rewards, actions, behavior_logprobs,
    ...
    if args.drA_loss_coef > 0.0:
      ...
      drA_loss = jnp.zeros_like(pg_loss)
  ...
  def run_policy_phase_single_device(
    ...
    def policy_phase_epoch(carry, _):
      def policy_phase_minibatch(carry, minibatch):
  ...
  def auxiliary_phase_loss_fn(params, target_params, obs, next_obs, dones, rewards, target_values, target_logits, actions, key):
    new_logits, _, reps_a = get_logits_auxvaluepred_rep(params.actor_params, obs)
    ...
    aux_phase_loss_critic = args.vf_coef * vloss + args.mico_coef_critic_aux_phase * mico_loss + \
                            args.markov_coef_c * dyna_loss_c + args.drC_loss_coef * drC_loss
