...
class Args:
  ...
  num_aux_minibatches: int = 8
  "the number of mini-batches in the auxiliary phase"
  gradient_accumulation_steps: int = 1
  "the number of gradient accumulation steps before performing an optimization step"
  num_policy_phases: int = 1
  ...
  vf_coef: float = 0.5
  "coefficient for the value loss during the auxiliary phase"
  ...
  # DAAC arguments
  adv_loss_coef_pol_phase: float = 0.0
  "coefficient for the advantage loss applied to the actor representation during policy phase"
  adv_loss_coef_aux_phase: float = 0.0
  ...
  # to prevent numerical instability, this will be set true if no losses are applied to the actor during the aux phase.
...
def make_ppg_model(args, envs, key, print_model=False):
  key, actor_base_key, policy_head_key, auxiliary_advantage_head_key, dyna_head_actor_key, \
  critic_base_key, value_head_key = jax.random.split(key, 7)
  ...
  actor_params = ActorParams(
    ...
    auxiliary_advantage_head.init(auxiliary_advantage_head_key, hidden_sample_actor, np.array([envs.single_action_space.sample()])),
...
class ActorParams:
  ...
  auxiliary_advantage_head_params: flax.core.FrozenDict
...
def rollout(
  ...
  def get_model_outputs(
    ...
    aux_adv_pred = AdvantageHead(envs.single_action_space.n).apply(params.actor_params.auxiliary_advantage_head_params,
                                                                   actor_base, action)
    return obs, action, logprob, value_pred.squeeze(), aux_adv_pred.squeeze(), key
  ...
  for update in range(initial_policy_version + 1, args.num_updates + 2):
    ...
    for _ in range(0, args.num_steps):
      ...
      cached_next_obs, action, logprob, value_pred, aux_adv_pred, key = \
      ...
      storage.append(
        PolPhaseStorage(
          ...
          aux_advantages=jax.lax.stop_gradient(aux_adv_pred),
          env_ids=env_id,
          rewards=next_reward,
...
if __name__ == "__main__":
  ...
  assert args.aux_batch_size % args.num_aux_minibatches == 0, "aux_batch_size must be divisible by num_aux_minibatches"
  args.aux_minibatch_size = args.aux_batch_size // args.num_aux_minibatches
  assert args.aux_minibatch_size % args.num_steps == 0, "aux_minibatch_size must be divisible by num_steps"
  args.num_rollouts_in_aux_mb = args.aux_minibatch_size // args.num_steps
  args.num_updates = args.total_timesteps // (args.local_batch_size * args.world_size)
  args.baseline_score = constants.PROCGEN_BASELINE_SCORES['ppo']['test_scores'][args.env_id]
  args.no_actor_grads_aux_phase = True if (
      args.bc_coef == 0.0 and
      args.markov_coef_a == 0.0 and
      args.mico_coef_actor_aux_phase == 0.0
  ) else False
  ...
  def policy_phase_loss_fn(params, target_params, obs, next_obs, rewards, actions, behavior_logprobs,
    ...
    #Advantage loss
    if args.adv_loss_coef_pol_phase > 0.0:
      adv_pred = get_auxadv(params.actor_params, reps_a, actions)
      adv_loss = ((adv_pred - advantages) ** 2).mean()
    else:
      adv_loss = jnp.zeros_like(entropy_loss)
    ...
    loss = pg_loss - args.ent_coef * entropy_loss + args.adv_loss_coef_pol_phase * adv_loss \
    ...
    stats = {
      ...
      'losses/advantage_loss': adv_loss,
    }
  ...
  def run_policy_phase_single_device(
    ...
    return agent_state, stats_pol_phase, jnp.hsplit(target_values, num_sharded_storages), jnp.hsplit(advantages, num_sharded_storages), key
  ...
  def auxiliary_phase_loss_fn(params, target_params, obs, next_obs, dones, rewards, target_values, target_logits, actions,
                              target_advantages, key):
    new_logits, reps_a = get_logits_rep(params.actor_params, obs)
    ...
    if args.adv_loss_coef_aux_phase > 0.0 or args.adv_loss_coef_pol_phase > 0.0:
      adv_pred = get_auxadv(params.actor_params, reps_a, actions)

    if args.adv_loss_coef_aux_phase > 0.0:
    ...
    aux_phase_loss_actor = args.bc_coef * kl_policy_bc + args.markov_coef_a * dyna_loss_a + \
                           args.mico_coef_actor_aux_phase * mico_loss_a + args.adv_loss_coef_aux_phase * adv_loss
    aux_phase_loss_critic = args.vf_coef * vloss + args.mico_coef_critic_aux_phase * mico_loss

    aux_phase_loss = aux_phase_loss_actor + aux_phase_loss_critic

    if args.mico_coef_critic_aux_phase > 0.0 or args.mico_coef_actor_aux_phase > 0.0:
      new_target_p = jax.tree_map(update_target_params, target_params, params)
    else:
      new_target_p = target_params
  ...
  def run_auxiliary_phase_single_device(
    ...
    def aux_phase_epoch(carry, _):
      ...
      def aux_phase_minibatch(carry, minibatch):
        ...
        grads = jax.lax.pmean(grads, axis_name="local_devices")
        if args.no_actor_grads_aux_phase:
